...
```bash
python3 {baseDir}/scripts/bootstrap_project.py --help
python3 {baseDir}/scripts/project_setup.py --help
```
...
```bash
python3 {baseDir}/scripts/project_state.py --help
```
...
```bash
python3 {baseDir}/scripts/project_audit.py PROJECT
python3 {baseDir}/scripts/requirements_lint.py PROJECT
python3 {baseDir}/scripts/traceability_check.py PROJECT
python3 {baseDir}/scripts/domain_audit.py PROJECT
```
...
```bash
python3 {baseDir}/scripts/change_manager.py --help
python3 {baseDir}/scripts/baseline_manager.py --help
python3 {baseDir}/scripts/configuration_audit.py PROJECT --strict
```
...
```bash
python3 {baseDir}/scripts/verification_evidence.py --help
python3 {baseDir}/scripts/verification_evidence_audit.py PROJECT --strict
```
...
```bash
python3 {baseDir}/scripts/management_audit.py PROJECT
python3 {baseDir}/scripts/management_audit.py PROJECT --release
```
...
```bash
python3 {baseDir}/scripts/release_check.py PROJECT
```
...
## 8. Validate changes to this skill

After modifying scripts, templates, references, CI, or this file, run:

```bash
python3 {baseDir}/scripts/run_checks.py
```

Target: all tests pass, package check passes, and self-check reports zero WARN
and zero FAIL. The detailed governance rules and Definitions of Done remain in
`references/project-manager-handbook.md`; consult only the sections needed for
the current project. Remaining capability gaps and decision-gated work are
tracked in `docs/INDUSTRIALIZATION-ROADMAP.md`.

## 9. Prepare public distribution

Read `docs/PUBLICATION.md`. Export only a reviewed clean skill snapshot using
`scripts/export_public_skill.py`, with a local private denylist when appropriate.
Inspect the manifest and scan using `scripts/publication_check.py`. Never mirror
private Git history, real project setup, workspace memory or authentication files
to a public repository. Publication requires the intended destination and user
authorization; preparing a local package does not publish it.
